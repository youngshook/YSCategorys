...
- (NSString *)macAddress {
	...
	NSLog(@"Get mac address failed: if_nametoindex failure");
	return nil;
    }

    // Get the size of the data available (store in len)
    if (sysctl(mgmtInfoBase, 6, NULL, &length, NULL, 0) < 0) {
	NSLog(@"Get mac address failed: sysctl mgmtInfoBase failure");
	return nil;
    }

    // Alloc memory based on above call
    if ((msgBuffer = malloc(length)) == NULL) {
	NSLog(@"Get mac address failed: buffer allocation failure");
	return nil;
    }

    // Get system information, store in buffer
    if (sysctl(mgmtInfoBase, 6, msgBuffer, &length, NULL, 0) < 0) {
	NSLog(@"Get mac address failed: sysctl msgBuffer failure");
	free(msgBuffer);
	return nil;
    }
